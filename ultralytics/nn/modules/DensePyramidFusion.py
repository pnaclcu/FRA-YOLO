...
# DensePyramidFusion.py
# 把相对导入换成绝对导入
from ultralytics.nn.modules.conv import Conv
from ultralytics.nn.modules.block import C2f, C3, C3Ghost, C3k2
...
class AFPN_P2345(nn.Module):
    def __init__(self,
        ...
        self.conv00 = Conv(in_channels[0] // factor, out_channels[0], 1)
        self.conv11 = Conv(in_channels[1] // factor, out_channels[1], 1)
        self.conv22 = Conv(in_channels[2] // factor, out_channels[2], 1)
        self.conv33 = Conv(in_channels[3] // factor, out_channels[3], 1)
        # init weight
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                nn.init.xavier_normal_(m.weight, gain=0.02)
...
if __name__ == '__main__':
    if __name__ == '__main__':
        x = [
            ...
            torch.randn(1, 768, 20, 20),  # P5
        ]

        model = AFPN_P2345_Custom(
            in_channels=[128, 256, 512, 768],
            out_channels=[128, 256, 512, 768],
            block_type='C2f',
            factor=4
        )

        outs = model(x)  # 返回 4 层特征
        for i, feat in enumerate(outs):
            print(f'out[{i}] shape: {feat.shape}')